...
public:
    ...
    class spell_hun_steady_shot_SpellScript : public SpellScript
    {
        ...
        void HandleImprovedSteady()
        {
            if (Unit* caster = GetCaster())
            {
                // Improved Steady Shot
                if (AuraEffect* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_HUNTER, 3409, EFFECT_0))
                {
                    if (caster->GetLastSpell() == SPELL_HUNTER_STEADY_SHOT)
                    {
    ...
    private:
        ...
        void Register()
        {
            AfterCast += SpellCastFn(spell_hun_steady_shot_SpellScript::HandleImprovedSteady);
    ...
    class spell_hun_serpent_sting_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Unit* target = GetTarget())
            {
                ...
                if (AuraEffect const* iss = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_HUNTER, 536, EFFECT_0))
                {
                    // Serpent Sting
                    if (AuraEffect* serpentSting = target->GetAuraEffect(GetSpellInfo()->Id, EFFECT_0, caster->GetGUID()))
                    {
                        bp0 += (serpentSting->GetAmount() * serpentSting->GetTotalTicks()) * iss->GetAmount() / 100;
                        bp0 += caster->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.4f / serpentSting->GetTotalTicks();

                        if (bp0 > 0)
                            caster->CastCustomSpell(target, 83077, &bp0, NULL, NULL, true, NULL, serpentSting->GetBase()->GetEffect(EFFECT_0));
                    }
                }
            }
        }
...
class spell_hun_rapid_killing : public SpellScriptLoader
{
public:
    spell_hun_rapid_killing() : SpellScriptLoader("spell_hun_rapid_killing")
    {
    }
    ...
    {
        PrepareAuraScript(spell_hun_rapid_killing_AuraScript);
