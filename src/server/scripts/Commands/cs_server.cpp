...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand serverCommandTable[] =
        {
            ...
            { "version",        SEC_PLAYER,         true,  &HandleServerVersionCommand,             "", NULL },
            { "dbversion",      SEC_PLAYER,         true,  &HandleServerDBVersionCommand,           "", NULL },
            { "uptime",         SEC_PLAYER,         true,  &HandleServerUptimeCommand,              "", NULL },
            { "players",        SEC_PLAYER,         true,  &HandleServerPlayersCommand,             "", NULL },
            { "users",          SEC_PLAYER,         true,  &HandleServerUsersCommand,               "", NULL },
            { "diff",           SEC_PLAYER,         true,  &HandleServerDiffCommand,                "", NULL },
    ...
    static bool HandleServerInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        handler->PSendSysMessage("|cffCD3333    by NorthStrider, John and Locknes|r\n");
    ...
    // External SOAP Commands
    static bool HandleServerVersionCommand(ChatHandler* handler, const char* /*args*/)
    {
        handler->PSendSysMessage(_FULLVERSION);
        return true;
    }

    static bool HandleServerDBVersionCommand(ChatHandler* handler, const char* /*args*/)
    {
        handler->PSendSysMessage(LANG_USING_WORLD_DB,sWorld->GetDBVersion());
        return true;
    }

    static bool HandleServerUptimeCommand(ChatHandler* handler, const char* /*args*/)
    {
        std::string uptime = secsToTimeString(sWorld->GetUptime());
        handler->PSendSysMessage(LANG_UPTIME, uptime.c_str());
        return true;
    ...
    static bool HandleServerPlayersCommand(ChatHandler* handler, const char* /*args*/)
    {
        uint32 playersNum = sWorld->GetPlayerCount();
        uint32 maxPlayersNum = sWorld->GetMaxPlayerCount();
        handler->PSendSysMessage(LANG_CONNECTED_PLAYERS, playersNum, maxPlayersNum);
    ...
    static bool HandleServerUsersCommand(ChatHandler* handler, const char* /*args*/)
    {
        uint32 activeClientsNum = sWorld->GetActiveSessionCount();
        uint32 queuedClientsNum = sWorld->GetQueuedSessionCount();
        uint32 maxActiveClientsNum = sWorld->GetMaxActiveSessionCount();
    ...
    static bool HandleServerDiffCommand(ChatHandler* handler, const char* /*args*/)
    {
        uint32 updateTime = sWorld->GetUpdateTime();
        handler->PSendSysMessage(LANG_UPDATE_DIFF, updateTime);
        return true;
