...
public:
    ...
    */
    ...
    static bool HandleLevelUpCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (uint32(newlevel) > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            newlevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
        else if (newlevel > STRONG_MAX_LEVEL)                         // hardcoded maximum level
            newlevel = STRONG_MAX_LEVEL;

        if (newlevel == handler->GetSession()->GetPlayer()->getLevel())
            return false;
