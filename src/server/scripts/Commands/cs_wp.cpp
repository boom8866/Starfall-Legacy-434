...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand wpCommandTable[] =
        {
            ...
            { "export",         SEC_ADMINISTRATOR,  false, &HandleWpExportCommand,             "", NULL },
            { "clear",          SEC_ADMINISTRATOR,  false, &HandleWpClearCommand,              "", NULL },
    ...
    */
    static bool HandleWpAddCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!pathid)
            pathid = target->GetDBTableGUIDLow() * 10;
        ...
        player->SummonCreature(1/*Waypoint Marker*/, *player, TEMPSUMMON_TIMED_DESPAWN, 45000);
    ...
    static bool HandleWpExportCommand(ChatHandler* handler, const char* args)
    {
        ...
        QueryResult result = WorldDatabase.PQuery("SELECT MAX(point) FROM waypoint_data WHERE id = '%u'", lowguid*10);
        if (result)
            maxpoint = (*result)[0].GetUInt32();
        else
            maxpoint = 0;

        result = WorldDatabase.PQuery("SELECT  `point`,  `position_x`,  `position_y`,  `position_z` FROM `waypoint_data` WHERE id = %u ORDER BY point", lowguid*10);

        if (!result)
        {
            handler->PSendSysMessage("No Waypoint for the selected creature exists");
            return false;
        }

        sLog->outInfo(LOG_FILTER_SQL_DEV, "-- Adds Waypoint Movement to %s (guid: %u, entry: %u)", creature->GetName().c_str(), lowguid, creature->GetEntry());
        sLog->outInfo(LOG_FILTER_SQL_DEV, "SET @GUID  := %u;\nSET @WP_ID := %u;\nUPDATE `creature` SET `MovementType` = 2, `spawndist` = 0 WHERE `guid` = @GUID;", lowguid, lowguid*10);

        CreatureAddonHelper::HandleNpcSetAddonQueryBuilder(CreatureAddonHelper::ADDON_CREATURE, creature->GetEntry(), lowguid, CreatureAddonHelper::ADDON_CNAME_PATHID, uint32(lowguid * 10), true);

        sLog->outInfo(LOG_FILTER_SQL_DEV, "DELETE FROM `waypoint_data` WHERE `id` = @WP_ID;\nINSERT INTO `waypoint_data` (`id`, `point`, `position_x`, `position_y`, `position_z`, `delay`) VALUES");
        ...
        do
        {
            ...
            sLog->outInfo(LOG_FILTER_SQL_DEV, "(@WP_ID, %u, %.3f, %.3f, %.3f, 0)%s", point, x, y ,z, (point == maxpoint) ? ";\n" : ",");
            count++;
        }
        while (result->NextRow());

        WorldDatabase.DirectPExecute("UPDATE `creature` SET `MovementType` = 2, `spawndist` = 0 WHERE `guid` = %u", lowguid);

        handler->PSendSysMessage("Exported %u waypoints.", count);
        handler->PSendSysMessage("Load Waypoints...");

        sWaypointMgr->ReloadPath(lowguid*10);

        creature->LoadPath(lowguid*10);
        creature->SetDefaultMovementType(WAYPOINT_MOTION_TYPE);
        creature->GetMotionMaster()->Initialize();
        creature->MonsterSay("Path loaded.", 0, 0);

        handler->PSendSysMessage("Done.", count);
        return true;
    }

    static bool HandleWpClearCommand(ChatHandler* handler, const char* args)
    {
        Creature* creature = handler->getSelectedCreature();
        if (!creature)
        {
        ...
        {
            WorldDatabase.DirectPExecute("DELETE FROM `waypoint_data` WHERE `id` = %u", lowguid*10);
            handler->PSendSysMessage("Deleted %u waypoints. (Didn't write action into devlog.)", (*result)[0].GetUInt32());
        }
        else
            handler->PSendSysMessage("No Waypoints exist, skipped.");
