...
public:
    ...
    static bool HandleModifyAlternativeCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (addmoney < 0)
        {
            ...
            else
            {
                ...
                if (handler->needReportToTarget(target))
                    (ChatHandler(target->GetSession())).PSendSysMessage(LANG_YOURS_MONEY_TAKEN, handler->GetNameLink().c_str(), abs(addmoney));
        ...
        else
        {
            ...
            if (handler->needReportToTarget(target))
                (ChatHandler(target->GetSession())).PSendSysMessage(LANG_YOURS_MONEY_GIVEN, handler->GetNameLink().c_str(), addmoney);
    ...
    static bool HandleModifyHonorCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_HONOR_POINTS, (amount * CURRENCY_PRECISION), true, true);
