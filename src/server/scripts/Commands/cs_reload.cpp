...
public:
    ...
    {
        sLog->outInfo(LOG_FILTER_GENERAL, "Re-Loading anticheat config table...");
    ...
    {
        sLog->outInfo(LOG_FILTER_GENERAL, "Re-Loading creature reward definitions...");
    ...
    static bool HandleReloadPerfConfigCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Reloading PerformanceLog config");
        sPerfLog->LoadConfig();
        handler->SendGlobalGMSysMessage("PerformanceLog config reloaded");
#else
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "PerformanceLog is disabled!");
#endif
        return true;
    }
};
