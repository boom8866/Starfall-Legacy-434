...
public:
    ...
    struct boss_vanessa_vancleefAI : public BossAI
    {
        ...
        void EnterCombat(Unit* who)
        {
            ...
            events.ScheduleEvent(EVENT_SHADOWGUARD, 9000);
            events.ScheduleEvent(EVENT_BACKSLASH, 15000);

            if (!me)
                return;

            me->MonsterYell(COMBAT_START, 0, PlayerGUID);
        ...
        void JustSummoned(Creature* summoned)
        {
            ...
            {
                case NPC_ROPE: summoned->SummonCreature(NPC_ROPE_ANCHOR, summoned->GetPositionX(), summoned->GetPositionY(), summoned->GetPositionZ()+40.0f, 0, TEMPSUMMON_TIMED_DESPAWN, 10000);
                    break;
            }
        ...
        void FieryBoom()
        {
            ...
            me->GetCreatureListWithEntryInGrid(fiery, 47242, 150.0f);
        ...
        void DamageTaken(Unit* done_by, uint32 & damage)
        {
            ...
            if (me->GetHealth() <= damage)
            {
                ...
                if (!Killed)
                {
                    me->MonsterTextEmote("Vanessa pulls out a final barrel of mining powder and ignites it! RUN!", PlayerGUID, true);
                    me->RemoveAllAuras();
                    me->AttackStop();
        ...
        {
            bunny = me->SummonCreature(45979, -52.31f, -820.18f, 51.91f, 3.32963f);
        ...
        void RopeReady()
        {
            me->MonsterTextEmote("Vannesa has detonated charges on the ship! Get to the ropes at the side of the boat!", PlayerGUID, true);

            std::list<Creature*> ropes;
            me->GetCreatureListWithEntryInGrid(ropes, 49550, 100.0f);
            ropes.sort(Trinity::ObjectDistanceOrderPred(me));
            for (std::list<Creature*>::iterator itr = ropes.begin(); itr != ropes.end(); itr++)
            {
                if((*itr)->isAlive() && (*itr)->GetTypeId() == TYPEID_UNIT)
                {
                    (*itr)->AddAura(SPELL_CLICK_ME, (*itr));
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (HealthBelowPct(51) && !Under)
            {
                events.ScheduleEvent(EVENT_DISSAPEAR, 1000);
                Under = true;
            }

            if (HealthBelowPct(26) && !Under2)
            {
                events.ScheduleEvent(EVENT_DISSAPEAR, 1000);
                Under2 = true;
            }
...
public:
    ...
    struct npc_defias_shadowguardAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            {
                DoCast(me->getVictim(), SPELL_SINISTER);
...
public:
    ...
    struct npc_oaf_lackeyAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            {
                DoCast(me->getVictim(), SPELL_AXE_HEAD);
...
public:
    ...
    struct npc_defias_bloodwizardAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            {
                DoCast(me->getVictim(), SPELL_BLOODBOLT);
            ...
            {
                DoCast(me->getVictim(), SPELL_RAGEZONE);
        ...
        void EjectPlayer()
        {
            for (uint8 i = 0; i<5; ++i)
            {
                ...
                if (Unit* players = me->GetVehicleKit()->GetPassenger(i))
                {
                    ...
                    me->MonsterTextEmote("The Nightmare Elixir takes hold!", PlayerGUID, true);
                    me->DespawnOrUnsummon(3000);
                }
            }
                ...
                if (Pulltimer <= diff)
                {
                    ...
                    for (std::list<Player*>::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    {
                        ...
                        if (Vehicle* vehicle = me->GetVehicleKit())
                            me->NearTeleportTo(-205.7569f, -579.0972f, 42.98623f, 2.3f);   
                        Pullplayers = false;
                    }
                }
                else Pulltimer -= diff;
...
class npc_note : public CreatureScript
{
    ...
    {
        if (player)
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, INTRUDER_SAY, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
public:
    ...
    struct npc_vanessa_nightmareAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            me->MonsterSay(VANESSA_GLUB_1, 0, 0);
            Nightmare = true;
            ShiftToTwo = false;
            ShiftToThree = false;
            ShiftToFour = false;
                ...
                if (NightmareTimer <= diff)
                {
                    switch (Phase)
                    {
                        case 9:
                        {
                            SummonSparks();
                            me->SetVisible(true);
                            me->MonsterSay(VANESSA_MECHANICAL_1, 0, 0);
            ...
            if (ShiftToFour)
            {
                if (NightmareTimer <= diff)
                {
                    switch (Phase)
                    {
                        ...
                        case 15: me->MonsterTextEmote("You have entered Ripsnarl's nightmare!", 0, true); NightmareTimer = 2000; Phase++; break;
                        case 16:
                        {
                            std::list<Player*> players;
...
public:
    ...
    struct npc_glubtok_dmAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            instance->SetData(DATA_NIGHTMARE_MECHANICAL, NOT_STARTED);
        }
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            instance->SetData(DATA_NIGHTMARE_MECHANICAL, IN_PROGRESS);
        }
...
public:
    ...
    struct npc_helix_dmAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            instance->SetData(DATA_NIGHTMARE_HELIX, IN_PROGRESS);
        }
