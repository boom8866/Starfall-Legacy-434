...
public:
    ...
    {
        npc_gilneas_city_guard_phase2AI(Creature* creature) : ScriptedAI(creature), tYell(urand(10000, 60000)) {}
...
public:
    ...
    struct npc_frightened_citizenAI : public ScriptedAI
    {
        ...
        Paths LoadPaths()
        {
            ...
            {
                auto const wp = sWaypointMgr->GetPath(349815 + i);
                paths.pointsCount[i] = wp->size();
                uint8 j = 0;

                for (auto point : *wp)
                {
                    paths.paths[i][j].x = point->x;
                    paths.paths[i][j].y = point->y;
                }
            }
            return paths;
        }
...
public:
    ...
    struct npc_bloodfang_worgenAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit* unit, SpellInfo const* info)
        {
            ...
            switch (info->Id)
            {
                ...
                // Eviscerate
                case 2098:
                    unit->ToPlayer()->KilledMonsterCredit(44175);
                    break;
            }
        }
    };
...
public:
    ...
    struct npc_sean_dempseyAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            tSummon          = 0;
            tEvent_Timer     = 0;
            tWave_Time       = urand(17000, 22000); // How often we spawn
        }
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
    {
        creature->CastSpell(player, SPELL_FORCECAST_GREYMANES_HORSE, true);
        return true;
    }
...
public:
    ...
    struct npc_bloodfang_stalker_c1AI : public ScriptedAI
    {
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell)
        {
            Unit* horse = caster->GetVehicleBase();
