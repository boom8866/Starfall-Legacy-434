...
enum Spells // Non-commented spells work completely. 88919 to 88922 visuals before encounter.
{
    ...
    /***** FELUDIUS *****/ 
    // ATTACK PHASE - ToDo: Script Frost Imbued.
    ...
    /***** IGNACIOUS *****/
    // ATTACK PHASE - ToDo: Script Flame Imbued.
...
public:
    ...
    struct npc_ascendant_council_controllerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            Creature* feludius = me->FindNearestCreature(BOSS_FELUDIUS, 500.0f, true); 
            Creature* ignacious = me->FindNearestCreature(BOSS_IGNACIOUS, 500.0f, true); 
            Creature* arion = me->FindNearestCreature(BOSS_ARION, 500.0f, true); 
            Creature* terrastra = me->FindNearestCreature(BOSS_TERRASTRA, 500.0f, true);  
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_MONSTROSITY:
                            ...
                            ignacious->SetDisplayId(MODEL_INVISIBLE);
					    	me->SummonCreature(BOSS_ELEMENTIUM_MONSTROSITY, -1008.824f, -582.617f, 831.902f, 0.026f, TEMPSUMMON_MANUAL_DESPAWN);
                            break;
                    ...
                    switch (eventId)
                    {
                        case EVENT_CHECK_WINDS_PLAYER:
                            if(Unit* victim = me->FindNearestPlayer(3.0f, true))
                                if(victim)
                                {
                                    me->AddAura(SPELL_SWIRLING_WINDS, victim);
                                    if(victim->HasAura(SPELL_GROUNDED))
                                        victim->RemoveAura(SPELL_GROUNDED);
                                }
