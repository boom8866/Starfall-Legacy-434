...
public:
    ...
    {
        boss_valiona_botAI(Creature* creature) : BossAI(creature, DATA_THERALION_AND_VALIONA), summons(me)
        {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            if (instance)
            {
                instance->SetBossState(DATA_THERALION_AND_VALIONA, IN_PROGRESS);
                instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, me); // Add
            }

            castBlackout = false;

            if (Creature* Theralion = me->FindNearestCreature(BOSS_THERALION_BOT, 500.0f, true))
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if (instance)
            {
                instance->SetBossState(DATA_THERALION_AND_VALIONA, DONE);
                instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me); // Remove
            }
...
public:
    ...
    {
        boss_theralion_botAI(Creature* creature) : BossAI(creature, DATA_THERALION_AND_VALIONA), summons(me)
        {
            instance = creature->GetInstanceScript();
        }
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            if (Creature* Valiona = me->FindNearestCreature(BOSS_VALIONA_BOT, 500.0f, true))
        ...
        void initEvents(bool onGround = true)
        {
            ...
            if(onGround)
            {
                ...
                events.ScheduleEvent(EVENT_LIFTOFF, 140000, PHASE_GROUND);
                if(me->GetMap()->IsHeroic())
                    events.ScheduleEvent(EVENT_TWILIGHT_SHIFT, 20000, PHASE_GROUND);
            }else
            {
                events.ScheduleEvent(EVENT_TWILIGHT_BLAST, 10000, PHASE_FLIGHT);
                events.ScheduleEvent(EVENT_DAZZLING_DESTRUCTION, 85000, PHASE_FLIGHT);
                events.ScheduleEvent(EVENT_LAND, 100000, PHASE_FLIGHT);
            }
        }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_ENGULFING_MAGIC:
                            ...
                            if (me->GetMap()->IsHeroic())
                            {
                                for (int32 i = 0; i < RAID_MODE(3, 4); ++i)
 			                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 200.0f, true))
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_DAZZLING_DESTRUCTION: // 2 sec timer, summon 6 then cast.
                        {
                            ...
                            if (uiDazzlingDestructionCount < MAX_DAZZLING_DESTRUCTION_CASTS)
                            {
                                ...
                                events.ScheduleEvent(EVENT_DAZZLING_DESTRUCTION, 4100, PHASE_FLIGHT);
                            }
                            else
                            {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_UNSTABLE_TWILIGHT:
                        if (me->FindNearestPlayer(8.0f, true))
                        {
