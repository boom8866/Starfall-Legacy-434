...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sPowerDisplayStore,           dbcPath, "PowerDisplay.dbc"); // 15595
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sResearchFieldStore,          dbcPath, "ResearchField.dbc");//14545
    ...
    for (uint32 i = 1; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        if (SpellCategoriesEntry const* category = sSpellCategoriesStore.LookupEntry(spell->SpellCategoriesId))
            sSpellCategoryMap[category->Category].insert(i);
    }
    ...
    // prepare fast data access to bit pos of talent ranks for use at inspecting
    {
        ...
        for (uint32 talentTabId = 1; talentTabId < sTalentTabStore.GetNumRows(); ++talentTabId)
        {
            ...
            {
                if (uint32 spellid = talentTabInfo->masterySpell[i])
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sTransportAnimationStore,     dbcPath, "TransportAnimation.dbc");// 15595
