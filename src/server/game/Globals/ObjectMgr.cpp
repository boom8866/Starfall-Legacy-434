...
bool ObjectMgr::GetPlayerGuildIDByGUID(uint64 guid, uint32 &gguid) const
{
    // prevent DB access for online player
    if (Player* player = ObjectAccessor::FindPlayer(guid))
    {
        gguid = player->GetGuildId();
        return true;
    }

    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_GUILDID);
    stmt->setUInt32(0, GUID_LOPART(guid));
    PreparedQueryResult result = CharacterDatabase.Query(stmt);

    if (result)
    {
        gguid = (*result)[0].GetUInt32();
        return true;
    }
...
void ObjectMgr::LoadRewardOnKill()
{
    ...
    do
    {
        ...
        if (rewOnKill.RepFaction1)
        {
            ...
            if (!factionEntry1)
            {
                sLog->outError(LOG_FILTER_SQL, "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reputation`", rewOnKill.RepFaction1);
                continue;
            }
        }
        ...
        if (rewOnKill.RepFaction2)
        {
            ...
            if (!factionEntry2)
            {
                sLog->outError(LOG_FILTER_SQL, "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reputation`", rewOnKill.RepFaction2);
                continue;
            }
        }
        ...
        if (rewOnKill.currencyid1)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.currencyid1);
                continue;
            }
        }
        ...
        if (rewOnKill.currencyid2)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.currencyid2);
                continue;
            }
        }
        ...
        if (rewOnKill.currencyid3)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CurrencyType (CurrencyTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.currencyid3);
                continue;
            }
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature reward definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureInvolvedRelations()
{
    ...
    // Needed for SMSG_QUEST_NPC_QUERY_RESPONSE
    _creatureInvolvedNpcsPerQuest.clear();
    for (QuestRelations::const_iterator itr = _creatureQuestInvolvedRelations.begin(); itr != _creatureQuestInvolvedRelations.end(); ++itr)
        _creatureInvolvedNpcsPerQuest[itr->second].insert(itr->first);
}
...
void ObjectMgr::LoadContentEmulation()
{
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Initialize Content emulation...");

    uint32 const prevEvent = _gameEmulationInfo.eventEntry;
    uint32 const gameBuild = sWorld->getIntConfig(CONFIG_EMULATED_GAMEBUILD);

    QueryResult result = WorldDatabase.PQuery("SELECT `version`, `maxItemEntry`, `eventEntry`, `welcomeMessage` FROM `content_emulation` WHERE `gamebuild` = %u", gameBuild);
    Field* fields = result->Fetch();
    ASSERT(fields && "Current emulated gamebuild is not valid!")

    _gameEmulationInfo.version          = fields[0].GetString();
    _gameEmulationInfo.maxItemEntry     = fields[1].GetUInt32();
    _gameEmulationInfo.eventEntry       = fields[2].GetUInt32();
    _gameEmulationInfo.welcomeMessage   = fields[3].GetUInt32();

    if (prevEvent != _gameEmulationInfo.eventEntry)
    {
        sGameEventMgr->StopEvent(prevEvent, true);
        sGameEventMgr->StartEvent(_gameEmulationInfo.eventEntry, true);
    }

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Emulated build is %s (%u).", _gameEmulationInfo.version.c_str(), gameBuild);
}

bool const ObjectMgr::SatisfyItemForEmulatedBuild(ItemTemplate const* proto, Player const* player) const
{
    // @ToDo: Check safety of recursive calls

    // Helper Class for some Checks
    class EmulationChecks
    {
    public:
        static bool CheckAuraOrSpellEffectForSpellInfo(SpellInfo const* info, Player const* player)
        {
            if (!info)
                return false;
            ...
            {
                // SpellEffects
                if (info->Effects[effect].Effect != 0)
                {
                    // Checks for some spell effects
                    switch (info->Effects[effect].Effect)
                    {
                        case SPELL_EFFECT_LEARN_SPELL:
                        {
                            if (!CheckAuraOrSpellEffectForSpellInfo(sSpellMgr->GetSpellInfo(info->Effects[effect].TriggerSpell), player))
                                return false;
                        ...
                        {
                            if (!sObjectMgr->SatisfyItemForEmulatedBuild(sObjectMgr->GetItemTemplate(info->Effects[effect].ItemType), player))
                                return false;
                ...
                // SpellAuraEffects
                if (info->Effects[effect].ApplyAuraName != 0)
                {
                    switch (info->Effects[effect].ApplyAuraName)
                    {
    ...
    if (player && (player->GetSession()->GetSecurity() >= SEC_MODERATOR) && sWorld->getBoolConfig(CONFIG_EMULATED_IGNORE_GAMEMASTER))
        return true;

    if (_gameEmulationInfo.maxItemEntry && proto->ItemId > _gameEmulationInfo.maxItemEntry)
    {
        switch (proto->Class)
        {
            // Item classes to check
        ...
        for (uint8 proto_spell = 0; proto_spell < MAX_ITEM_PROTO_SPELLS; ++proto_spell)
            if (proto->Spells[proto_spell].SpellId)
                if (!EmulationChecks::CheckAuraOrSpellEffectForSpellInfo(sSpellMgr->GetSpellInfo(proto->Spells[proto_spell].SpellId), player))
                    return false;
    }
    return true;

    // proto->ItemLevel < sWorld->getIntConfig(CONFIG_EMULATED_REQUIRED_ITEMLEVEL)
}

void ObjectMgr::LoadAntiCheatConfig()
{
    QueryResult result  = WorldDatabase.Query("SELECT checktype, check_period, alarmscount, disableoperation, messagenum,"
                                              "intparam1, intparam2, floatparam1, floatparam2, action1, actionparam1,"
                                              "action2, actionparam2, description FROM anticheat_config");

    m_AntiCheatConfig.clear();
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 anticheat config definitions");
        sLog->outError(LOG_FILTER_SQL, "Error loading `anticheat_config` table or table is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint32 const checkType = fields[0].GetUInt32();
        if (checkType >= CHECK_MAX)
        {
            sLog->outError(LOG_FILTER_SQL, "Wrong check type id %u in `anticheat_config` table, ignoring.", checkType);
            continue;
        }

        AntiCheatConfig conf;
        conf.checkType          = checkType;
        conf.checkPeriod        = fields[1].GetUInt32();
        conf.alarmsCount        = fields[2].GetUInt32();
        conf.disableOperation   = fields[3].GetBool();
        conf.messageNum         = fields[4].GetUInt32();
        conf.checkParam[0]      = fields[5].GetUInt32();
        conf.checkParam[1]      = fields[6].GetUInt32();
        conf.checkFloatParam[0] = fields[7].GetFloat();
        conf.checkFloatParam[1] = fields[8].GetFloat();
        conf.actionType[0]      = fields[9].GetUInt32();
        conf.actionParam[0]     = fields[10].GetUInt32();
        conf.actionType[1]      = fields[11].GetUInt32();
        conf.actionParam[1]     = fields[12].GetUInt32();
        conf.description        = fields[13].GetString();

        m_AntiCheatConfig[AntiCheatCheck(checkType)] = conf;

        ++count;
    }
    while (result->NextRow());

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u anticheat config definitions", count);
}

AntiCheatConfig const* ObjectMgr::GetAntiCheatConfig(uint32 const checkType) const
{
