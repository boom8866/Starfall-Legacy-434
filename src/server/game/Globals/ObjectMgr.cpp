...
void ObjectMgr::LoadCreatureTemplate(Field* fields)
{
    ...
    creatureTemplate.trainer_class = uint32(fields[35].GetUInt8());
    creatureTemplate.trainer_race = uint32(fields[36].GetUInt8());
    creatureTemplate.type = uint32(fields[37].GetUInt8());
    creatureTemplate.type_flags = fields[38].GetUInt32();
    creatureTemplate.type_flags2 = fields[39].GetUInt32();
...
void ObjectMgr::LoadVehicleTemplateAccessories()
{
    ...
    do
    {
        ...
        if (!sObjectMgr->GetCreatureTemplate(entry))
        {
            sLog->outError(LOG_FILTER_SQL, "Table `vehicle_template_accessory`: creature template entry %u does not exist.", entry);
            continue;
        }

        if (!sObjectMgr->GetCreatureTemplate(accessory))
        {
            sLog->outError(LOG_FILTER_SQL, "Table `vehicle_template_accessory`: Accessory %u does not exist.", accessory);
            continue;
        }

        if (_spellClickInfoStore.find(entry) == _spellClickInfoStore.end())
        {
            sLog->outError(LOG_FILTER_SQL, "Table `vehicle_template_accessory`: creature template entry %u has no data in npc_spellclick_spells", entry);
            continue;
        }
    ...
    do
    {
        ...
        switch (got.type)
        {
            ...
            case GAMEOBJECT_TYPE_TRANSPORT: // 11
            {
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Gameobject (Entry: %u GoType: %u) is transport but does not have entries in TransportAnimation.dbc! Gameobject is obsolete.",
                        got.entry, got.type);
                    break;
                }
...
{
    LoadQuestRelationsHelper(_goQuestRelations, nullptr, "gameobject_questrelation", true, true);
...
{
    LoadQuestRelationsHelper(_goQuestInvolvedRelations, &_goQuestInvolvedRelationsReverse, "gameobject_involvedrelation", false, true);
...
{
    LoadQuestRelationsHelper(_creatureQuestRelations, nullptr, "creature_questrelation", true, false);
...
{
    LoadQuestRelationsHelper(_creatureQuestInvolvedRelations, &_creatureQuestInvolvedRelationsReverse, "creature_involvedrelation", false, false);
