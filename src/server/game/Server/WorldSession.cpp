...
void WorldSession::SendSuspendComms()
{
...
void WorldSession::ReadMovementInfo(WorldPacket& data, MovementInfo* mi, ExtraMovementInfo* emi)
{
    ...
    //sLog->outInfo(LOG_FILTER_SERVER_LOADING, "%f %f %f", mi->pos.m_positionX, mi->pos.m_positionY, mi->pos.m_positionZ);
    ...
    { \
        ...
        { \
            sLog->outDebug(LOG_FILTER_UNITS, "WorldSession::ReadMovementInfo: Violation of MovementFlags found (%s). " \
                "MovementFlags: %u, MovementFlags2: %u for player GUID: %lu. Mask %u will be removed.", \
                STRINGIZE(check), mi->GetMovementFlags(), mi->GetExtraMovementFlags(), mi->guid, maskToRemove); \
    ...
    */
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_HOVER) && !_player->HasAuraType(SPELL_AURA_HOVER),
        MOVEMENTFLAG_HOVER);
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_WATERWALKING) && !_player->HasAuraType(SPELL_AURA_WATER_WALK),
        MOVEMENTFLAG_WATERWALKING);

    //! Cannot feather fall without SPELL_AURA_FEATHER_FALL
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_FALLING_SLOW) && !_player->HasAuraType(SPELL_AURA_FEATHER_FALL),
    ...
    */
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_FLYING | MOVEMENTFLAG_CAN_FLY) && GetSecurity() == SEC_PLAYER &&
        !_player->HasAuraType(SPELL_AURA_FLY) &&
