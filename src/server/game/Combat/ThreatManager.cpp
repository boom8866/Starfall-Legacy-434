...
HostileReference* ThreatContainer::selectNextVictim(Creature* attacker, HostileReference* currentVictim) const
{
    ...
    for (ThreatContainer::StorageType::const_iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found;)
    {
        ...
        if (!noPriorityTargetFound && (target && target->IsInWorld() && attacker && attacker->IsInWorld() && target->IsImmunedToDamage(attacker->GetMeleeDamageSchoolMask()) || (target && target->IsInWorld() && target->HasNegativeAuraWithInterruptFlag(AURA_INTERRUPT_FLAG_TAKE_DAMAGE))))
        {
            if (iter != lastRef)
            {
