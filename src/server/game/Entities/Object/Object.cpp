...
WorldObject::~WorldObject()
{
    ...
    if (IsWorldObject() && m_currMap)
    {
        ...
        {
            sLog->outFatal(LOG_FILTER_GENERAL, "Object::~Object Corpse guid=%lu, type=%d, entry=%u deleted but still in map!!",
...
Object::~Object()
{
    if (IsInWorld())
    {
        sLog->outFatal(LOG_FILTER_GENERAL, "Object::~Object - guid=%lu, typeid=%d, entry=%u deleted but still in world!!", GetGUID(), GetTypeId(), GetEntry());
    ...
    if (m_objectUpdated)
    {
        sLog->outFatal(LOG_FILTER_GENERAL, "Object::~Object - guid=%lu, typeid=%d, entry=%u deleted but still in update list!!", GetGUID(), GetTypeId(), GetEntry());
        ASSERT(false);
...
bool WorldObject::canSeeOrDetect(WorldObject const* obj, bool ignoreStealth, bool distanceCheck) const
{
    ...
    if (obj->IsInvisibleGMDueToDespawn(this))
        return false;
...
bool WorldObject::CanDetect(WorldObject const* obj, bool ignoreStealth) const
{
    ...
    if (obj->ToUnit() && obj->ToUnit()->isSummon())
    {
        ...
        {
            if (summoner->ToTempSummon())
            {
                if (seer == summoner->ToTempSummon()->GetSummoner())
                    return true;
            }
        }
    }
...
void Unit::BuildHeartBeatMsg(WorldPacket* data) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->GetSession()->WriteMovementInfo(*data);
}
    ...
    if (summoner)
    {
...

        {
            ...
            for (auto effect : list)
                if (effect->GetBase()->IsPermanent())
                    summon->CastSpell(summon, effect->GetId(), TRIGGERED_FULL_MASK);
        }

        {
            auto list = summoner->GetAuraEffectsByType(SPELL_AURA_MOD_INVISIBILITY_DETECT);
            for (auto effect : list)
                if (effect->GetBase()->IsPermanent())
                    summon->CastSpell(summon, effect->GetId(), TRIGGERED_FULL_MASK);
        }
