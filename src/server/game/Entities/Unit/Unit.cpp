    ...
    if (Aura* aura = aur->GetBase())
    {
        if (Player *player = ToPlayer())
        {
            if (player->HaveSpectators() && slot < MAX_AURAS)
            {
                ...
                msg.CreateAura(casterID, aura->GetSpellInfo()->Id, aura->GetSpellInfo()->IsPositive(), aura->GetSpellInfo()->Dispel, aura->GetDuration(), aura->GetMaxDuration(), aura->GetStackAmount(), false);
        ...
        if (Aura* aura = aurApp->GetBase())
        {
            if (Player *player = ToPlayer())
            {
                if (player->HaveSpectators())
                {
                    ...
                    msg.SetPlayer(player->GetName());
                    msg.CreateAura(casterID, aura->GetSpellInfo()->Id, aurApp->IsPositive(), aura->GetSpellInfo()->Dispel, aura->GetDuration(), aura->GetMaxDuration(), aura->GetStackAmount(), true);
                    player->SendSpectatorAddonMsgToBG(msg);
                }
            }
        }
