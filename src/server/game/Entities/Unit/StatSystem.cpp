...
void Player::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    SetModifierValue(unitMod_neg, BASE_VALUE, 0);
    ...
    float attPowerMultiplier = GetModifierValue(unitMod_pos, TOTAL_PCT) - GetModifierValue(unitMod_neg, TOTAL_PCT);
    ...
    if (!ranged)
    {
        ...
        for (AuraEffectList::const_iterator iter = mAPbyArmor.begin(); iter != mAPbyArmor.end(); ++iter)
            ...
            attPowerMod_pos += int32(GetArmor() / (*iter)->GetAmount());
    }
...
void Player::UpdateRuneRegen(RuneType rune)
{
    ...
    if (uint32 cooldown = GetRuneTypeBaseCooldown(rune))
        SetFloatValue(PLAYER_RUNE_REGEN_1 + uint8(rune), float(IN_MILLISECONDS) / float(cooldown));
}
...
void Player::UpdateMasteryAuras()
{
    ...
    {
        Aura *aura = GetAura(masterySpells->at(i));
        if (!aura)
            continue;
...
{
    UnitMods unitModPos = ranged ? UNIT_MOD_ATTACK_POWER_RANGED_POS : UNIT_MOD_ATTACK_POWER_POS;
    UnitMods unitModNeg = ranged ? UNIT_MOD_ATTACK_POWER_RANGED_NEG : UNIT_MOD_ATTACK_POWER_NEG;
    ...
    float baseAttackPower  = (GetModifierValue(unitModPos, BASE_VALUE) - GetModifierValue(unitModNeg, BASE_VALUE)) * (GetModifierValue(unitModPos, BASE_PCT) + (1 - GetModifierValue(unitModNeg, BASE_PCT)));
    float attackPowerModPos = GetModifierValue(unitModPos, TOTAL_VALUE);
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    if (m_owner->HasAura(76657) && m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        AddPct(mindamage, mindamage * (0.0167f * masteryPoints) * 0.50f);
        AddPct(maxdamage, maxdamage *(0.0167f * masteryPoints) * 0.50f);
    }
...
void Guardian::ApplyRatingMod(CombatRating cr, float ratingChange, bool apply)
{
    ...
    {
        case CR_HASTE_MELEE:
            SetHastePct(ratingChange, CTYPE_BASE);
            SetHastePct(ratingChange, CTYPE_OFF);
            break;
        case CR_HASTE_RANGED:
            SetHastePct(ratingChange, CTYPE_RANGED);
            break;
        case CR_HASTE_SPELL:
