...
class Unit : public WorldObject
{
    public:
        ...
        void PrepareKill(Unit* victim, bool durabilityLoss = false);
        ...
        */
        ...
        void RemoveFoodBuffAurasWithExclusion(uint32 excludeSpellId);
        void RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, uint64 casterGUID);
        void RemoveAurasWithMechanic(uint32 mechanic_mask, AuraRemoveMode removemode = AURA_REMOVE_BY_DEFAULT, uint32 except=0);
        void RemoveOneAuraWithMechanic(uint32 mechanic_mask, AuraRemoveMode removemode = AURA_REMOVE_BY_DEFAULT, uint32 except=0);
        ...
        void RemoveAllAurasExceptType(AuraType type1, AuraType type2, AuraType type3);
        void DelayOwnedAuras(uint32 spellId, uint64 caster, int32 delaytime);
        void RemoveRespecAuras();
        ...
        float CalculateResistPct(Unit* victim, SpellSchoolMask schoolMask);
        ...
        // Quest: Graduation Speech
        uint8 m_graduationSpeechInspire;
    ...
    // HasteSystem
    public:
        void InitializeCombatSpeed();
        void UpdateCombatSpeedMod(CombatType cmbt);

        void SetHastePct(float hastePct, CombatType cmbt);
        void ApplyCombatSpeedPctMod(CombatType cmbt, float val, bool apply);
        void SetBaseAttackTime(WeaponAttackType att, uint32 time);
        uint32 GetBaseAttackTime(WeaponAttackType att) const { return m_baseCombatTime[att]; }
        float GetTotalCombatSpeedMod(CombatType cmbt) const { return m_modTotalCombatSpeed[cmbt]; }
        float GetHasteMod(CombatType cmbt) const { return m_hasteMod[cmbt]; }
    private:
        uint32 m_baseCombatTime[MAX_ATTACK];
        float m_hasteMod[MAX_CTYPE];
        float m_modCombatSpeed[MAX_CTYPE];
        float m_modTotalCombatSpeed[MAX_CTYPE];
};
