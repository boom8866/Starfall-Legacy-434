                    ...
                    switch (info->entry)
                    {
                        ...
                        case 184142:                        // Netherstorm Flag
                            if (bg->GetTypeID(true) == BATTLEGROUND_EY)
                                bg->EventPlayerClickedOnFlag(player, this);
                            break;
                        case 179830:                        // Twin Peaks Alliance Flag
                        case 179831:                        // Twin Peaks Horde Flag
...
void GameObject::CastSpell(Unit* target, uint32 spellId)
{
    ...
    if (Unit* owner = GetOwner())
    {
        ...
        if (target)
        {
            if (!trigger->canSeeOrDetect(target))
            {
                target->CastSpell(target, spellInfo, true);
                return;
            }
...
void GameObject::ActivateAnimation(uint32 anim) // where the hell is this used and how? Should it be used anywhere as a function?
{
