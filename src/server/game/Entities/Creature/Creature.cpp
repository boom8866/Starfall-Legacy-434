...
m_creatureInfo(NULL), m_creatureData(NULL), m_path_id(0), m_formation(NULL), uiSeerGUID(0)
{
...
bool Creature::UpdateEntry(uint32 Entry, uint32 team, const CreatureData* data)
{
    ...
    SetBaseAttackTime(RANGED_ATTACK, cInfo->BaseAttackTime);
...
void Creature::Update(uint32 diff)
{
    ...
    switch (GetZoneId())
    {
        ...
        case 4815:  // Vashj'ir
        {
            ...
            if (GetZoneId() == 16 && (GetCreatureTemplate()->expansion == 3 || GetCreatureTemplate()->Entry == 35867))
                break;
...
void Creature::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        ...
        if (GetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID)) // if creature is mounted on a virtual mount display id, remove it
...
std::string CreatureAddonHelper::GetTableName(AddonType const type)
{
    std::string const tableName[ADDON_MAX] = {"creature_addon", "creature_template_addon"};
    return tableName[type];
}

std::string CreatureAddonHelper::GetPrimaryKeyName(AddonType const type)
{
    std::string const tableName[ADDON_MAX] = {"guid", "entry"};
    return tableName[type];
}

std::string CreatureAddonHelper::GetShortTableName(AddonType const type)
{
    std::string const tableName[ADDON_MAX] = {"ca", "cta"};
    return tableName[type];
}

std::string CreatureAddonHelper::GetColumnName(AddonColumnName const column)
{
    std::string const columnName[ADDON_CNAME_MAX] = {"path_id", "mount", "bytes1", "bytes2", "emote", "auras"};
    return columnName[column];
}

bool CreatureAddonHelper::FillAddonOf(CreatureAddon& addon, AddonType const table, uint32 const entryOrGuid)
{
    QueryResult result = WorldDatabase.PQuery("SELECT path_id, mount, bytes1, bytes2, emote, auras FROM %s WHERE %s = %u", GetTableName(table).c_str(), GetPrimaryKeyName(table).c_str(), entryOrGuid);

    if (result)
    {
        Field* fields = result->Fetch();

        addon.path_id = fields[0].GetUInt32();
        addon.mount   = fields[1].GetUInt32();
        addon.bytes1  = fields[2].GetUInt32();
        addon.bytes2  = fields[3].GetUInt32();
        addon.emote   = fields[4].GetUInt32();
        ConvertStringToAuras(addon.auras, fields[5].GetString());
        return true;
    }
    return false;
}

void CreatureAddonHelper::ConvertStringToAuras(std::vector<uint32>& auras, std::string const& str)
{
    if (str.empty())
        return;

    Tokenizer tokens(str, ' ');
    auras.resize(tokens.size());
    for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        auras.push_back(atol(*itr));
}

std::string CreatureAddonHelper::ConvertAurasToString(std::vector<uint32> const& auras)
{
    if (auras.empty())
        return "NULL";

    std::stringstream query;
    for (auto const val : auras)
        query << val << " ";

    return "\'" + query.str().substr(0, query.str().size() - 1) + "\'";
}

bool CreatureAddonHelper::HandleNpcSetAddonQueryBuilder(AddonType const tableName, uint32 const entry, uint32 const guid, AddonColumnName const column, int64 const value, bool const isInlineCall)
{
    CreatureAddon addon;
    addon.path_id = 0;
    addon.mount   = 0;
    ...
    bool const addonFound = FillAddonOf(addon, tableName, (tableName == ADDON_CREATURE) ? guid : entry);
    if (!addonFound && tableName == ADDON_CREATURE)
        FillAddonOf(addon, ADDON_CREATURE_TEMPLATE, entry);

    switch (column)
    {
        case ADDON_CNAME_PATHID:
            addon.path_id = value;
            break;
        case ADDON_CNAME_MOUNT:
            addon.mount = value;
            break;
        case ADDON_CNAME_BYTES1:
            addon.bytes1 = value;
            break;
        case ADDON_CNAME_BYTES2:
            addon.bytes2 = value;
            break;
        case ADDON_CNAME_EMOTE:
            addon.emote = value;
            break;
        case ADDON_CNAME_AURA:
            {
                if (value > 0)
                    addon.auras.push_back(value);
                else if (value < 0)
                {
                    for (auto itr = addon.auras.begin(); itr != addon.auras.end(); ++itr)
                        if (*itr == abs(value))
                        {
                            addon.auras.erase(itr);
                            break;
                        }
                }
                else if (value == 0)
    ...
    if (!isInlineCall)
        if (CreatureTemplate const* proto = sObjectMgr->GetCreatureTemplate(entry))
        {
            std::stringstream query;
            query << "-- Updates the addon of " << proto->Name << " ( Entry: " << entry;
            if (tableName == ADDON_CREATURE)
                query << " Guid: " << guid;
            query << " )";

            sLog->outInfo(LOG_FILTER_SQL_DEV, query.str().c_str());
        }

    if (addonFound)
    {
        std::stringstream query;
        query << "UPDATE `" << GetTableName(tableName) << "` SET `" << GetColumnName(column) << "` = ";
        if (column == ADDON_CNAME_AURA)
            query << ConvertAurasToString(addon.auras);
        else
            query << value;

        query << " WHERE `" << GetPrimaryKeyName(tableName) << "` = " << ((tableName == ADDON_CREATURE) ? guid : entry) << ";";

        WorldDatabase.DirectExecute(query.str().substr(0, query.str().size() - 1).c_str());

        if (isInlineCall)
            sLog->outInfo(LOG_FILTER_SQL_DEV, query.str().c_str());
        else
            sLog->outInfo(LOG_FILTER_SQL_DEV, "%s\n", query.str().c_str());
    }
    else
    {
        {
            std::stringstream query;
            query << "DELETE FROM `" << GetTableName(tableName) << "` WHERE `" << GetPrimaryKeyName(tableName) << "` = " << ((tableName == ADDON_CREATURE) ? guid : entry) << ";";
        ...
        {
            std::stringstream query;
            query << "INSERT INTO `" << GetTableName(tableName) << "` (`" << GetPrimaryKeyName(tableName)
                << "`, `path_id`, `mount`, `bytes1`, `bytes2`, `emote`, `auras`) VALUES";

            dbquery.append(query.str() + " ");
            sLog->outInfo(LOG_FILTER_SQL_DEV, query.str().c_str());
        }

        {
            ...
            query << "(" << ((tableName == ADDON_CREATURE) ? guid : entry) << ", "
                << addon.path_id << ", "
                << addon.mount << ", "
                << addon.bytes1 << ", "
                << addon.bytes2 << ", "
                << addon.emote << ", "
                << ConvertAurasToString(addon.auras) << ");";
        ...
        WorldDatabase.DirectExecute(dbquery.substr(0, dbquery.size() - 1).c_str());
    }

    CreatureAddon const* addonptr;
    if (tableName == ADDON_CREATURE)
        addonptr = sObjectMgr->GetCreatureAddon(guid);
    else
        addonptr = sObjectMgr->GetCreatureTemplateAddon(entry);

    if (addonptr)
    {
        CreatureAddon* ad = const_cast<CreatureAddon*>(addonptr);
        ad->path_id = addon.path_id;
        ad->mount   = addon.mount;
        ad->bytes1  = addon.bytes1;
        ad->bytes2  = addon.bytes2;
        ad->emote   = addon.emote;
        ad->auras   = addon.auras;
        return true;
    }
    return false;
}
