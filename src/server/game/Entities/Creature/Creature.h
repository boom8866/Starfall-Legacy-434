...
// Static Class to update addon data easily
class CreatureAddonHelper
{
public:
    enum AddonType
    {
        ADDON_CREATURE,
        ADDON_CREATURE_TEMPLATE,
        ADDON_MAX
    };

    enum AddonColumnName
    {
        ADDON_CNAME_PATHID,
        ADDON_CNAME_MOUNT,
        ADDON_CNAME_BYTES1,
    ...
    static std::string GetTableName(AddonType const type);

    static std::string GetPrimaryKeyName(AddonType const type);

    static std::string GetShortTableName(AddonType const type);

    static std::string GetColumnName(AddonColumnName const column);

    static bool FillAddonOf(CreatureAddon& addon, AddonType const table, uint32 const entryOrGuid);

    static void ConvertStringToAuras(std::vector<uint32>& auras, std::string const& str);

    static std::string ConvertAurasToString(std::vector<uint32> const& auras);

    static bool HandleNpcSetAddonQueryBuilder(AddonType const tableName, uint32 const entry, uint32 const guid, AddonColumnName const column, int64 const value, bool const isInlineCall);
};
...
#define MAX_VENDOR_ITEMS 600                                // Limitation in 4.x.x item count in SMSG_LIST_INVENTORY
...
class Creature : public Unit, public GridObject<Creature>, public MapCreature
{
    ...
    protected:
        ...
        void HandleInhabitType(uint32 const& InhabitType);
    private:
