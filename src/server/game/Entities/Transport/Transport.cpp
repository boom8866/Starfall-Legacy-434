...
void MapManager::LoadTransports()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT guid, entry, phasemask, name, period, ScriptName FROM transports");
    ...
    do
    {
        ...
        uint32 period = fields[4].GetUInt32();
        ...
        if (!t->Create(lowguid, entry, phasemask, mapid, x, y, z, o, 255, 0))
        {
            delete t;
            continue;
        }
...
void MapManager::LoadTransportNPCs()
{
    ...
    //                                                 0       1            2              3             4             5             6        7             8
    QueryResult result = WorldDatabase.Query("SELECT guid, npc_entry, transport_entry, phasemask, TransOffsetX, TransOffsetY, TransOffsetZ, TransOffsetO, emote FROM creature_transport");
    ...
    do
    {
        ...
        uint32 phasemask = fields[3].GetUInt32();
        float tX = fields[4].GetFloat();
        float tY = fields[5].GetFloat();
        float tZ = fields[6].GetFloat();
        float tO = fields[7].GetFloat();
        uint32 anim = fields[8].GetInt32();

        for (MapManager::TransportSet::iterator itr = m_Transports.begin(); itr != m_Transports.end(); ++itr)
        {
            if ((*itr)->GetEntry() == transportEntry)
            {
                if ((*itr)->GetPhaseMask() & phasemask)
                    (*itr)->AddNPCPassenger(guid, entry, phasemask, tX, tY, tZ, tO, anim);
                else
                    sLog->outError(LOG_FILTER_SQL, "Transport Npcs (entry: %u, guid: %u) has an invalid phasemask because it can't detect his transport in some phases.", entry, guid);
                break;
            }
        }
...
bool Transport::Create(uint32 guidlow, uint32 entry, uint32 phasemask, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress, uint32 dynflags)
{
    Relocate(x, y, z, ang);
...
uint32 Transport::AddNPCPassenger(uint32 tguid, uint32 entry, uint32 phasemask, float x, float y, float z, float o, uint32 anim)
{
    Map* map = GetMap();
