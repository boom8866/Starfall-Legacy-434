...
// Reset Battle Ress System
void InstanceScript::DoResetBressSystem()
{
    Map::PlayerList const &PlayerList = instance->GetPlayers();
    if (!PlayerList.isEmpty())
        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            if (Player* player = i->getSource())
                player->m_bressCount = 0;
}
...
bool const InstanceScript::AreAllPlayersDead(bool const& checkOoC) const
{
    Map::PlayerList const& players = instance->GetPlayers();
    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
        if ((itr->getSource()->isAlive() && (!checkOoC || itr->getSource()->isInCombat())) && !itr->getSource()->isGameMaster())
            return false;
