...
void Guild::Member::SaveToDB(SQLTransaction& trans) const
{
    ...
    stmt->setUInt32(5, m_weekReputation);
    CharacterDatabase.ExecuteOrAppend(trans, stmt);
}
...
uint32 Guild::Member::CalculateReputationReward(uint32 const reputation)
{
    uint32 const cap = sWorld->getIntConfig(CONFIG_GUILD_WEEKLY_REP_CAP);

    if (m_weekReputation >= cap)
        return 0;
    else
    {
        m_weekReputation += reputation;
        if (m_weekReputation > cap)
            return m_weekReputation - cap;
        else
            return reputation;
    }
}
...
void Guild::SaveToDB()
{
    ...
    uint32 ccount = uint32(uint32(_challengecount[GUILD_CHALLENGE_DUNGEON]) | uint32(_challengecount[GUILD_CHALLENGE_RAID]) <<8 | uint32(_challengecount[GUILD_CHALLENGE_RATED_BATTLGROUND]) <<16 | uint32(0) <<24);
...
uint32 Guild::CalculateReputationRewardForMember(uint64 const& guid, uint32 const reputation)
{
    if (Member* member = GetMember(guid))
        return member->CalculateReputationReward(reputation);
    else
        return 0;
}
...
void Guild::HandleRoster(WorldSession* session)
{
    ...
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        memberData << uint32(session->GetPlayer()->GetReputation(GUILD_FACTION_ID));
...
void Guild::HandleSetEmblem(WorldSession* session, const EmblemInfo& emblemInfo)
{
    ...
    else
    {
        ...
        UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BUY_GUILD_TABARD, 1, 0, 0, NULL, session->GetPlayer());
        HandleQuery(session);
    }
}
...
void Guild::HandleBuyBankTab(WorldSession* session, uint8 tabId)
{
    ...
    UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BUY_GUILD_BANK_SLOTS, tabId, 0, 0, NULL, session->GetPlayer());
...
void Guild::SendChallengeUpdate(WorldSession* session) const
{
    WorldPacket data(SMSG_GUILD_CHALLENGE_UPDATED, 80);
    //Guild Experience Reward ???
    for(int i=0;i < GUILD_CHALLENGE_MAX; i++)
        data << uint32(challengeRewards[i].GuildXP);
    //Gold Reward Guild LEVEL 25
    for(int i=0;i < GUILD_CHALLENGE_MAX; i++)
        data << uint32(challengeRewards[i].GoldRewardHighLevel);
    //Total Count
    for(int i=0;i < GUILD_CHALLENGE_MAX; i++)
        data << uint32(challengeRewards[i].TotalCount);
    //Gold Reward Guild LEVEL < 25
    for(int i=0;i < GUILD_CHALLENGE_MAX; i++)
        data << uint32(challengeRewards[i].GoldRewardLowLevel);
    //Current Count
    for(int i=0;i < GUILD_CHALLENGE_MAX; i++)
        data << uint32(_challengecount[i]);
...
bool Guild::LoadFromDB(Field* fields)
{
    ...
    uint32 ccount = fields[16].GetUInt32();

    _challengecount[GUILD_CHALLENGE_DUNGEON]              = uint8((ccount)        & 0x0000FF);
    _challengecount[GUILD_CHALLENGE_RAID]                 = uint8((ccount >> 8)   & 0x0000FF);
    _challengecount[GUILD_CHALLENGE_RATED_BATTLGROUND]    = uint8((ccount >> 16)  & 0x0000FF);
...
bool Guild::AddMember(uint64 guid, uint8 rankId)
{
    ...
    if (player)
    {
        ...
        if (FactionEntry const* faction = sFactionStore.LookupEntry(GUILD_FACTION_ID))
            player->GetReputationMgr().SetReputation(faction, REP_NEUTRAL);
    }
    else
    {
        member->ResetFlags();
