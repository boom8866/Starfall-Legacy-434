...
SpellImplicitTargetInfo::StaticData SpellImplicitTargetInfo::_data[TOTAL_SPELL_TARGETS] =
{
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_SRC,    TARGET_SELECT_CATEGORY_AREA,    TARGET_CHECK_ENEMY,    TARGET_DIR_NONE},        // 126 - target only players
...
SpellEffectInfo::SpellEffectInfo(SpellEntry const* spellEntry, SpellInfo const* spellInfo, uint8 effIndex, SpellEffectEntry const* _effect)
{
    ...
    if (MaxRadiusEntry && RadiusEntry && (MaxRadiusEntry->RadiusMin < RadiusEntry->RadiusMax))
        MaxRadiusEntry = NULL;
        ...
        if (neededTargets & (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT_RAID | TARGET_FLAG_UNIT_PARTY | TARGET_FLAG_UNIT_MINIPET | TARGET_FLAG_UNIT_PASSENGER))
        {
            ...
            if (neededTargets & TARGET_FLAG_UNIT_PASSENGER)
                if (unitTarget->IsOnVehicle(caster))
                    return SPELL_CAST_OK;
            if (unitTarget->GetVehicleBase() && unitTarget->GetVehicleBase()->GetTypeId() == TYPEID_PLAYER)
                return SPELL_CAST_OK;
            return SPELL_FAILED_BAD_TARGETS;
        }
    }
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask) const
{
    ...
    if (AttributesEx4 & SPELL_ATTR4_SPELL_VS_EXTEND_COST)
        powerCost += caster->GetBaseAttackTime(OFF_ATTACK) / 100;
...
bool SpellInfo::IsMastery() const
{
...
int32 SpellInfo::GetMasteryCoefficient() const
{
    if (!IsMastery())
        return 0;

    // value for 76808 "Executioner" is not stored at all
    if (Id == 76808)
        return 250;
    ...
    {
        if (!Effects[j - 1].IsEffect())
            continue;

        if (int32 coef = Effects[j - 1].CalcValue())
            return coef;
    }
