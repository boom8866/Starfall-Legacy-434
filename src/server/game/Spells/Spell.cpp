...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    else if (m_damage > 0)
    {
        ...
        if (caster->GetTypeId() == TYPEID_PLAYER)
            caster->ToPlayer()->GetAntiCheat().DoAntiCheatCheck(CHECK_DAMAGE_SPELL, m_spellInfo->Id, NULL_OPCODE, damageInfo.damage);
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    else
    {
        ...
        if (!(_triggeredCastFlags & TRIGGERED_IGNORE_AURA_INTERRUPT_FLAGS) && m_spellInfo->IsBreakingStealth() && !GetCaster()->HasAura(51755))
        {
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            // Target must be facing you (excluding casters who have Glyph of Gouge active)
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            {
                // Exeption for underwater mounts like seehorses
                if (m_caster->IsInWater() && m_spellInfo->Id != 75207 && m_spellInfo->Id != 98718)
...
void Spell::Delayed() // only called in DealDamage()
{
    ...
    int32 delaytime = 300;                                  // spellcasting delay is normally 500ms
