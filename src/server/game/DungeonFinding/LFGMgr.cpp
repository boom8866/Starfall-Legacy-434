...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded %u lfg entrance positions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void LFGMgr::FinishDungeon(uint64 gguid, const uint32 dungeonId)
{
    ...
    for (LfgGuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (cta && rDungeonId != 300 && rDungeonId != 416 && rDungeonId != 417 && dungeon->expansion == EXPANSION_CATACLYSM)
        {
...
LfgLockMap const LFGMgr::GetLockedDungeons(uint64 guid)
{
    ...
    if (!player)
    {
        sLog->outWarn(LOG_FILTER_LFG, "Player: %u not ingame while retrieving his LockedDungeons.", GUID_LOPART(guid));
        return lock;
    }
