...
void ArchaeologyMgr::LoadData()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT id, mapId, questPOI, reqSkill, minLevel, goEntry FROM archaeology_sites");
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 archaeology sites. DB table `archaeology_site` is empty!");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u archaeology sites in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    for (uint32 i = 0; i < sQuestPOIPointStore.GetNumRows(); i++)
    {
        ...
        for (std::list<std::pair<uint16, uint32> >::iterator itr = sites.begin(); itr != sites.end();)
        {
            if (itr->second == entry->blobId)
            {
                ...
                if (_polygonMap[site])
                {
                    sLog->outError(LOG_FILTER_SERVER_LOADING, "Archaeology: Tried to overwrite polygonData of site %u", site);
                    break;
                }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u archaeology site polygons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u archaeology branches in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 archaeology projects. DB table `archaeology_projects` is empty!");
        return;
    }
    ...
    while (result->NextRow());
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u archaeology projects in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Calculated %u archaeology branch chances in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
{
    if (!_objectMap[entry])
        sLog->outError(LOG_FILTER_PLAYER_SKILLS, "Archaeology: Entry %u has no find object Type!", entry);
