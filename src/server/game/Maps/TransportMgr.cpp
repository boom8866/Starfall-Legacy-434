...
void TransportMgr::LoadTransportTemplates()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_TRANSPORTS, ">> Loaded 0 transport templates. DB table `gameobject_template` has no transports!");
        return;
    }
    ...
    do
    {
        ...
        if (goInfo == NULL)
        {
            sLog->outError(LOG_FILTER_TRANSPORTS, "Transport %u has no associated GameObjectTemplate from `gameobject_template` , skipped.", entry);
            continue;
        }

        if (goInfo->moTransport.taxiPathId >= sTaxiPathNodesByPath.size())
        {
            sLog->outError(LOG_FILTER_TRANSPORTS, "Transport %u (name: %s) has an invalid path specified in `gameobject_template`.`data0` (%u) field, skipped.", entry, goInfo->name.c_str(), goInfo->moTransport.taxiPathId);
            continue;
        }
    ...
    sLog->outInfo(LOG_FILTER_TRANSPORTS, ">> Loaded %u transport templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
Transport* TransportMgr::CreateTransport(uint32 entry, uint32 guid /*= 0*/, Map* map /*= NULL*/)
{
    ...
    if (!tInfo)
    {
        sLog->outError(LOG_FILTER_TRANSPORTS, "Transport %u will not be loaded, `transport_template` missing", entry);
        return NULL;
    }
    ...
    if (MapEntry const* mapEntry = sMapStore.LookupEntry(mapId))
    {
        ...
        {
            sLog->outError(LOG_FILTER_TRANSPORTS, "Transport %u (name: %s) attempted creation in instance map (id: %u) but it is not an instanced transport!", entry, trans->GetName().c_str(), mapId);
            delete trans;
            return NULL;
        }
    }
...
void TransportMgr::SpawnContinentTransports()
{
    ...
    sLog->outInfo(LOG_FILTER_TRANSPORTS, ">> Spawned %u continent transports in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
