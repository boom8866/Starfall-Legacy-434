...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetCharmInfo() && me->GetCharmInfo()->GetChasingUnit())
    {
        ...
        if (spellInfo->StartRecoveryCategory > 0 && me->GetCharmInfo() && me->GetCharmInfo()->GetGlobalCooldownMgr().HasGlobalCooldown(spellInfo))
...
void PetAI::HandleReturnMovement()
{
    ...
    if (me->GetCharmInfo() && me->GetCharmInfo()->GetCommandState() == COMMAND_MOVE_TO)
        return;

    if (me->GetCharmInfo() && me->GetCharmInfo()->HasCommandState(COMMAND_STAY))
    {
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    if (me->GetCharmInfo() && me->GetCharmInfo()->IsReturning())
