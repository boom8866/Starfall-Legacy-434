...
void WorldSession::HandleGuildRewardsQueryOpcode(WorldPacket& recvPacket)
{
    ...
    if (sGuildMgr->GetGuildById(_player->GetGuildId()))
    {
        ...
        {
            data << uint32(rew.second.Standing);
            data << int32(rew.second.Racemask);
            data << uint32(rew.second.Entry);
            data << uint64(rew.second.Price);
            data << uint32(0); // Unused
            data << uint32(rew.second.AchievementId);
        }
