...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    ...
    bool instantLogout = GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) || GetPlayer()->isInFlight() ||
    ...
    if (GetPlayer()->isInCombat() && !canLogoutInCombat)
        reason = 1;
    else if (GetPlayer()->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING | MOVEMENTFLAG_FALLING))
...
void WorldSession::HandleRealmSplitOpcode(WorldPacket& recvData)
{
    ...
    recvData >> clientstate;

    switch (clientstate)
    {
        case CLIENT_SPLIT_STATE_UNINITIALIZED:
        case CLIENT_SPLIT_STATE_NO_CHOICE:
        case CLIENT_SPLIT_STATE_REALM_1:
        case CLIENT_SPLIT_STATE_REALM_2:
            WorldPacket data(SMSG_REALM_SPLIT, 4+4+split_date.size()+1);
            data << clientstate;
            data << uint32(PENDING_SPLIT_STATE_NO_SERVER_SPLIT);                             // realm split state
            data << split_date;
            SendPacket(&data);
    }
...
void WorldSession::HandleGuildAchievementTracking(WorldPacket& recvData)
{
...
void WorldSession::SendSetPhaseShift(std::set<uint32> const& phaseIds, std::set<uint32> const& terrainswaps)
{
    ...
    data << uint32(phaseIds.empty() ? 8 : 0);  // flags (not phasemask)
