...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    {
        if (spellInfo->IsRaidMarker())
            sLog->outDebug(LOG_FILTER_NETWORKIO, "Received Raidmarker spell cast request.");
        else if (caster->ToPlayer()->HasArchProject(spellInfo->ResearchProject))
            sLog->outDebug(LOG_FILTER_NETWORKIO, "Received correct archaeology complete spell cast request.");
        else if (!caster->ToPlayer()->HasActiveSpell(spellId))
        {
...
void WorldSession::HandleCancelChanneling(WorldPacket& recvData)
{
    ...
    mover->InterruptSpell(CURRENT_CHANNELED_SPELL, true, true);
}
