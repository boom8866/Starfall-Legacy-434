...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    {
        if (Guild* guild = sGuildMgr->GetGuildById(pCurrChar->GetGuildId()))
            guild->SendLoginInfo(pCurrChar->GetSession());
        else
        {
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    uint32 tempRace = (1 << (race - 1));
    uint32 tempOldRace = (1 << (oldRace - 1));
    if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
        if ((tempRace & RACEMASK_ALLIANCE && tempOldRace & RACEMASK_ALLIANCE) || (tempRace & RACEMASK_HORDE && tempOldRace & RACEMASK_HORDE))
        {
            WorldPacket data(SMSG_CHAR_FACTION_CHANGE, 1);
