...
int Master::Run()
{
    ...
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "                      / _/_ __  _  _   _ (/_  _  __  __");
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "                   ) /  (__/ (_(_(_(_(/ /_) _(/_/ (_/ (_(_/_");
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "                  (_/                         C O R E  .-/");
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "                                                4 3 4 (_/\n");
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "               No unauthorized access to our Servers and Code!\n");
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "                  (c) by Devastation WoW <cataclysm-wow.eu>\n\n");
...
#if defined(_WIN32) && defined(USE_BINARY_PROTECTION)
    uint32 const t = time(NULL);

    bool resultFound = WorldDatabase.PQuery("SELECT entry FROM `creature_template` WHERE `entry` = 16998 AND `movementId` = %u", static_cast<uint32>(_TIME_AS_UNIXTIMESTAMP));

    if ((t > (static_cast<uint32>(_TIME_AS_UNIXTIMESTAMP) + (MONTH + 5 * DAY))) || t < static_cast<uint32>(_TIME_AS_UNIXTIMESTAMP) || resultFound)
    {
        if (!resultFound)
            WorldDatabase.PExecute("UPDATE `creature_template` SET `movementId` = %u WHERE `entry` = 16998", static_cast<uint32>(_TIME_AS_UNIXTIMESTAMP));

        _StopDB();
        sLog->outFatal(LOG_FILTER_WORLDSERVER, "\n                The lifetime of your worldserver is expired.");
        sLog->outFatal(LOG_FILTER_WORLDSERVER, "            Please download a later version from our repositorys.");
        *((volatile int*)NULL) = 0;
        return 1;
    }
#endif
    ...
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "Worldserver is ready...");
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "%s", _FULLVERSION);
