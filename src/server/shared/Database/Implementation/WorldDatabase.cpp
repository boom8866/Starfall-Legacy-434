...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_UPD_CREATURE_DAMAGE, "UPDATE creature_template SET DamageModifier = ? WHERE entry = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_CREATURE_ARMOR, "UPDATE creature_template SET ArmorModifier = ? WHERE entry = ?", CONNECTION_ASYNC);
